...
public class JiekuanApplyActivity_Yiban extends HeadBaseActivity {
    @BindView(R.id.name)
    TextView mName;
    ...
    protected void initView(RelativeLayout headView, RelativeLayout backBtn, RelativeLayout headerCenter, RelativeLayout headerRight,
        ...
        ((TextView) headerCenter.getChildAt(0)).setText("借款流程");
        initThisView();
    }
    ...
    private void RequestServer() {
        ...
        Queue.add(1, request, new OnResponseListener<ProcessShenheHistoryRes>() {
            ...
            public void onFailed(int what, Response<ProcessShenheHistoryRes> response) {
                Toast.makeText(JiekuanApplyActivity_Yiban.this, "请求数据失败", Toast.LENGTH_SHORT).show();
            }
    ...
    private void RequestServerCommit(String comment) {
        ...
        Queue.add(0, requestCommit, new OnResponseListener<ProcessJieguoResponse>() {
            ...
            public void onSucceed(int what, Response<ProcessJieguoResponse> response) {
                if (null != response && null != response.get()) {
                    if (response.get().getCode() == 200) {
                        Toast.makeText(JiekuanApplyActivity_Yiban.this, "流程审核成功", Toast.LENGTH_SHORT).show();
                        finish();
                    }
                }
            }
            ...
            public void onFailed(int what, Response<ProcessJieguoResponse> response) {
                Toast.makeText(JiekuanApplyActivity_Yiban.this, "流程审核失败", Toast.LENGTH_SHORT).show();
            }
...
//        if (!TextUtils.isEmpty(yijian)) {
            ...
            Queue.add(0, request, new OnResponseListener<ProcessJieguoResponse>() {
                ...
                public void onSucceed(int what, Response<ProcessJieguoResponse> response) {
                    if (null != response && null != response.get()) {
                        if (response.get().getCode() == 200) {
                            Toast.makeText(JiekuanApplyActivity_Yiban.this, "退回成功", Toast.LENGTH_SHORT).show();
                            finish();
                        }
                    }
                }
