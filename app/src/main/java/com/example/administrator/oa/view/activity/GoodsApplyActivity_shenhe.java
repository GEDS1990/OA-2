...
public class GoodsApplyActivity_shenhe extends HeadBaseActivity {
    ...
    protected void initView(RelativeLayout headView, RelativeLayout backBtn, RelativeLayout headerCenter,
        ...
        ((TextView) headerCenter.getChildAt(0)).setText("低值易耗品领用审核单");
        initThisView();
    }
...
//                        }
                    ...
                    for (int i = 0;i< goods.size();i++) {
                        ...
                        mGoodApplyAdapter.add(new GoodsRegistrationBean(goods.get(i), format.get(i), num.get(i), remarks.get(i)));
                    }
                }
            }
...
//        if (!TextUtils.isEmpty(yijian)) {
            ...
            Queue.add(0, request, new OnResponseListener<ProcessJieguoResponse>() {
                ...
                public void onSucceed(int what, Response<ProcessJieguoResponse> response) {
                    if (null != response && null != response.get()) {
                        if (response.get().getCode() == 200) {
                            Toast.makeText(GoodsApplyActivity_shenhe.this, "回退成功", Toast.LENGTH_SHORT).show();
                            finish();
                        }
                    }
                }
