...
//                    showFileChooser();
            ...
            case R.id.rl_fujian:
                ...
                if(!TextUtils.isEmpty(mFilePathReturn)){
                    ...
                    FileUtils.openLocalFile(ShouwenProcess_shenhe.this, mFilePath);
                }
                break;
        ...
        if (resultCode == RESULT_OK) {
            if (requestCode == REQUESTCODE_FROM_ACTIVITY) {
                ...
                if (Build.VERSION.SDK_INT > Build.VERSION_CODES.KITKAT) {//4.4以后
                    path = FileUtils.getPath(ShouwenProcess_shenhe.this, uri);
                } else {//4.4以下下系统调用方法
                    path = FileUtils.getRealPathFromURI(ShouwenProcess_shenhe.this, uri);
                }
    ...
    private void RequestServerForViewFile() {
        ...
        ProcessQueue.add(0, request, new OnResponseListener<FilePreviewResponse>() {
                ...
                if (null != response && null != response.get()) {
                    if (response.get().getCode() == 200) {
                        if(null != response.get().getData()) {
                            FileUtils.openWebFile(ShouwenProcess_shenhe.this, response.get().getData().getViewurl());
                        }
                    }
                }
            }
            ...
            public void onFailed(int what, Response<FilePreviewResponse> response) {
                Toast.makeText(ShouwenProcess_shenhe.this, "预览失败", Toast.LENGTH_SHORT).show();
            }
    ...
    private void RequestServerUploadFile(String picpath, String filename) {
        ...
        ProcessQueue.add(1, request, new OnResponseListener<ProcessJieguoResponse>() {
            ...
            public void onSucceed(int what, Response<ProcessJieguoResponse> response) {
                ...
                if (null != response && null != response.get() && null != response.get().getData()) {
                    Toast.makeText(ShouwenProcess_shenhe.this, "上传成功", Toast.LENGTH_SHORT).show();
//                mBtnCancel.setVisibility(View.VISIBLE);
            ...
            public void onFailed(int what, Response<ProcessJieguoResponse> response) {
                Toast.makeText(ShouwenProcess_shenhe.this, "上传失败", Toast.LENGTH_SHORT).show();
            }
