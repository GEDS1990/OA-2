...
public class Touzixieyi_shenhe extends HeadBaseActivity{
...
//                            .start();
            ...
            case R.id.rl_fujian:
                ...
                if(!TextUtils.isEmpty(mFilePathReturn)){
                    ...
                    FileUtils.openLocalFile(Touzixieyi_shenhe.this, mFilePath);
                }
                break;
    ...
    private void RequestServer() {
        ...
        Queue.add(1, request, new OnResponseListener<ProcessShenheHistoryRes>() {
            ...
            public void onFailed(int what, Response<ProcessShenheHistoryRes> response) {
                Toast.makeText(Touzixieyi_shenhe.this, "请求数据失败", Toast.LENGTH_SHORT).show();
            }
    ...
    private void RequestServerForViewFile() {
        ...
        ProcessQueue.add(0, request, new OnResponseListener<FilePreviewResponse>() {
                ...
                if (null != response && null != response.get()) {
                    if (response.get().getCode() == 200) {
                        if(null != response.get().getData()) {
                            FileUtils.openWebFile(Touzixieyi_shenhe.this, response.get().getData().getViewurl());
                        }
                    }
                }
            }
            ...
            public void onFailed(int what, Response<FilePreviewResponse> response) {
                Toast.makeText(Touzixieyi_shenhe.this, "预览失败", Toast.LENGTH_SHORT).show();
            }
    ...
    private void RequestServerDownLoad(){
        ...
        downloadQueue.add(0, downloadRequest, new DownloadListener() {
            ...
            public void onDownloadError(int what, Exception exception) {
                ...
                Toast.makeText(Touzixieyi_shenhe.this, "附件下载失败", Toast.LENGTH_SHORT).show();
            }
...
//                Log.w("2333", "mProgress=" + mProgress);
            ...
            public void onFinish(int what, String filePath) {
                ...
                Toast.makeText(Touzixieyi_shenhe.this, "附件下载完成", Toast.LENGTH_SHORT).show();
    ...
    private void RequestServerCommit(String comment) {
        ...
        Queue.add(0, requestCommit, new OnResponseListener<ProcessJieguoResponse>() {
            ...
            public void onSucceed(int what, Response<ProcessJieguoResponse> response) {
                if (null != response && null != response.get()) {
                    if (response.get().getCode() == 200) {
                        Toast.makeText(Touzixieyi_shenhe.this, "流程审核成功", Toast.LENGTH_SHORT).show();
                        finish();
                    }
                }
            }
            ...
            @Override
            public void onFailed(int what, Response<ProcessJieguoResponse> response) {
                Toast.makeText(Touzixieyi_shenhe.this, "流程审核失败", Toast.LENGTH_SHORT).show();
            }
...
//        if (!TextUtils.isEmpty(yijian)) {
            ...
            Queue.add(0, request, new OnResponseListener<ProcessJieguoResponse>() {
                ...
                public void onSucceed(int what, Response<ProcessJieguoResponse> response) {
                    if (null != response && null != response.get()) {
                        if (response.get().getCode() == 200) {
                            Toast.makeText(Touzixieyi_shenhe.this, "退回成功", Toast.LENGTH_SHORT).show();
                            finish();
                        }
                    }
                }
